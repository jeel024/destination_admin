import 'package:destination_admin/utils/Extensions/int_extensions.dart';
...
class CityWidgetState extends State<CityWidget> {
  ...
  async {
    await cityService
        .fetchCityList(list: cityList, stateId: stateId)
        .then((value) {
  ...
  Widget headingWidget() {
    return Row(
      children: [
        Text("City", style: boldTextStyle(size: 20, color: primaryColor)),
              Spacer(),
              SizedBox(
          width: 240,
...
16.width,
  ...
  changeStatus({required String id, required int status}) async {
    await cityService
        .updateDocument({CommonKeys.status: status}, id).then((value) async {
      await placeService.placesFuture(stateId: id).then((value) {
        value.forEach((element) async {
          await placeService
              .updateDocument({CommonKeys.status: status}, element.id);
        });
      });
    }).catchError((e) {
      toast(e.toString());
  ...
  deleteState(String id) async {
    await cityService.removeDocument(id).then((value) async {
      CityModel item = cityList.firstWhere((element) => element.id == id);
      await deleteFile(item.image.validate(), prefix: mStateStoragePath)
          .then((value) async {
        ...
        await placeService.placesFuture(stateId: id).then((value) {
          value.forEach((element) async {
            await placeService.removeDocument(element.id).then((value) async {
              await deleteFile(element.image.validate(),
                      prefix: mPlacesStoragePath)
                  .then((value) {
                if ((element.secondaryImages ?? []).isNotEmpty) {
                  Future.forEach(element.secondaryImages!, (element) async {
                    await deleteFile(element.toString(),
                            prefix: mPlacesStoragePath)
                        .then((value) {});
                  });
                }
          ...
          cityList.isNotEmpty
                            ...
                            child: SingleChildScrollView(
                              ...
                              child: ConstrainedBox(
                                ...
                                child: DataTable(
                                  ...
                                  rows: cityList.map((CityModel mData) {
                                    return DataRow(cells: [
                                      DataCell(Text(
                                          '${cityList.indexOf(mData) + 1}')),
                                      ...
                                      DataCell(
                                        Container(
                                          ...
                                          child: FutureBuilder<int>(
                                              future: placeService.totalPlacesByCity(
                                                  cityId: mData.id),
              ...
              : !appStore.isLoading
                  ? Column(
                      ...
                      .paddingAll(16)
                      .visible(!appStore.isLoading && cityList.isEmpty)
                  : SizedBox(),
