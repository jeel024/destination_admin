...
class AddCategoryDialog extends StatefulWidget {
  ...
  AddCategoryDialog({this.categoryModel, this.onUpdate});
...
class AddCategoryDialogState extends State<AddCategoryDialog> {
  ...
  Future<void> init() async {
    isUpdate = widget.categoryModel != null;
    if (isUpdate) {
      nameController.text = widget.categoryModel!.name.validate();
      status = widget.categoryModel!.status.validate(value: 1);
    }
  }
  ...
  Future<void> save() async {
    ...
    categoryData.name = nameController.text.trim();
    categoryData.updatedAt = DateTime.now();
    categoryData.status = status;

    if (isUpdate) {
      categoryData.id = widget.categoryModel!.id;
      categoryData.createdAt = widget.categoryModel!.createdAt;
      categoryData.image = widget.categoryModel!.image;
    } else {
      categoryData.createdAt = DateTime.now();
    }

    if (image != null) {
      await uploadFile(bytes: await image!.readAsBytes(), prefix: mCategoryStoragePath).then((path) async {
    ...
    if (isUpdate) {
      await categoryService.updateDocument(categoryData.toJson(), categoryData.id).then((value) {
        ...
        toast(language.categoryUpdated);
      }).catchError((e) {
        appStore.setLoading(false);
        toast(e.toString());
      });
  ...
  Widget getImage() {
    ...
    } else if (isUpdate && widget.categoryModel!.image.validate().isNotEmpty) {
      return cachedImage(widget.categoryModel!.image.validate(), height: 130, width: 130, fit: BoxFit.cover, alignment: Alignment.center);
    } else {
      return SizedBox();
    }
  }
    ...
    return AlertDialog(
      ...
      title: Container(
        ...
        child: Row(
          ...
          children: [
            Text(language.addCategory, style: boldTextStyle(color: primaryColor, size: 20)),
            IconButton(
      ...
      actions: <Widget>[
        ...
        dialogPrimaryButton(language.submit, () {
          if (formKey.currentState!.validate()) {
            if (image != null || (isUpdate && widget.categoryModel!.image!.isNotEmpty)) {
